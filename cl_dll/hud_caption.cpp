...
bool CHudCaption::Draw(float flTime)
{
	if (!sub_count)
		return false;

	const int lineHeight = CHud::UtfText::LineHeight();
	const int distanceBetweenSubs = V_max(lineHeight / 5, 1);

	const int xpos = SUB_START_XPOS;
	const int xmax = SUB_MAX_XPOS;
	// TO-DO: doesn't work with screen scaling! (prob)
	int ypos = ScreenHeight - gHUD.m_iFontHeight + gHUD.m_iFontHeight/2 - lineHeight - SUB_BORDER_LENGTH;
...
bool CHudCaption::ParseFloatParameter(char* pfile, int& currentTokenStart, unsigned int& tokenLength, Caption_t& caption)
{
	...
	char numbuf[8];
	strncpyEnsureTermination(numbuf, pfile + currentTokenStart, V_min(tokenLength+1, sizeof(numbuf)));
