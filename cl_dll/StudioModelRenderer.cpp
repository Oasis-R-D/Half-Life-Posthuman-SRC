...
//
//===========================================
char glsl_basic_depth[] = R"(
	#version 120

	uniform vec3 u_LightDir;     // program.local[0]
	...
	void main() {
		// Basic lighting
		float lightDot = max(dot(aNormal, u_LightDir), 0.0);
		vec3 lightResult = clamp(u_Ambient + u_LightColor * lightDot, 0.0, 1.0);

		vColor = vec4(lightResult, aColor.a);

		// Transform position
		vec4 modelPos = u_ModelMatrix * vec4(aPosition, 1.0);
		gl_Position = u_ViewProjMatrix * modelPos;

		// Fog uses depth (Z)
		vFogCoord = gl_Position.z;

		// Texture coordinate scaling
		vTexCoord = aTexCoord * u_TexScale;
	}
	)";
...
char glsl_frag_reg[] = R"(
	...
	varying vec2 vTexCoord; // fragment.texcoord[0]
	varying vec4 vColor; // fragment.color.primary
...
//	ARB SHADER
//
// TODO: convert all this arb assembly code into glsl,
// this takes up alot of space and is very hard to read, not to mention this
// language is ancient and is probably incompatible with some new gpus
//
//===========================================


char arb_basic_depth[] = // no model lights
	// program.local[0] = lightdir
	// program.local[1] = ambientlight
	// program.local[2] = diffuselight
	// program.local[3-6] = modelview matrix
	// program.local[7-10] = projection matrix
	//   program.local[11] = vector2d(1.0 / texturewidth, 1.0 / textureheight)
	"!!ARBvp1.0"
	"TEMP R0;"
...
char arb_1light_depth[] = // 1 model light
						  //  program.local[0] = lightdir
						  //  program.local[1] = ambientlight
						  //  program.local[2] = diffuselight
						  //  program.local[3-6] = modelview matrix
						  //  program.local[7-10] = projection matrix
						  //  program.local[11] = vector2d(1.0 / texturewidth, 1.0 / textureheight)
						  //  program.local[12] = vector4d(mdl_light[0]_origin.xyz, 1)
						  //  program.local[13] = vector4d(mdl_light[0]_color.xyz, mdl_light[0]_radius)
						  //  program.local[14] = vector4d(mdl_light[0]_forward.xyz,
						  //								cos((mdl_light[0]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_2lights_depth[] = // 2 model lights
						   //  program.local[0] = lightdir
						   //  program.local[1] = ambientlight
						   //  program.local[2] = diffuselight
						   //  program.local[3-6] = modelview matrix
						   ...
						   //  program.local[14] = vector4d(mdl_light[0]_forward.xyz,
						   //								cos((mdl_light[0]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
						   //  program.local[15] = vector4d(mdl_light[1]_origin.xyz, 1)
						   //  program.local[16] = vector4d(mdl_light[1]_color.xyz, mdl_light[1]_radius)
						   //  program.local[17] = vector4d(mdl_light[1]_forward.xyz,
						   //								cos((mdl_light[1]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_3lights_depth[] = // 3 model lights
						   //  program.local[0] = lightdir
						   //  program.local[1] = ambientlight
						   //  program.local[2] = diffuselight
						   //  program.local[3-6] = modelview matrix
						   ...
						   //  program.local[17] = vector4d(mdl_light[1]_forward.xyz,
						   //								cos((mdl_light[1]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
						   //  program.local[18] = vector4d(mdl_light[2]_origin.xyz, 1)
						   //  program.local[19] = vector4d(mdl_light[2]_color.xyz, mdl_light[2]_radius)
						   //  program.local[20] = vector4d(mdl_light[2]_forward.xyz,
						   //								cos((mdl_light[2]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_4lights_depth[] = // 4 model lights
						   //  program.local[0] = lightdir
						   //  program.local[1] = ambientlight
						   //  program.local[2] = diffuselight
						   //  program.local[3-6] = modelview matrix
						   ...
						   //  program.local[20] = vector4d(mdl_light[2]_forward.xyz,
						   //								cos((mdl_light[2]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
						   //  program.local[21] = vector4d(mdl_light[3]_origin.xyz, 1)
						   //  program.local[22] = vector4d(mdl_light[3]_color.xyz, mdl_light[3]_radius)
						   //  program.local[23] = vector4d(mdl_light[3]_forward.xyz,
						   //								cos((mdl_light[3]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_5lights_depth[] = // 5 model lights
						   //  program.local[0] = lightdir
						   //  program.local[1] = ambientlight
						   //  program.local[2] = diffuselight
						   //  program.local[3-6] = modelview matrix
						   ...
						   //  program.local[23] = vector4d(mdl_light[3]_forward.xyz,
						   //								cos((mdl_light[3]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
						   //  program.local[24] = vector4d(mdl_light[4]_origin.xyz, 1)
						   //  program.local[25] = vector4d(mdl_light[4]_color.xyz, mdl_light[4]_radius)
						   //  program.local[26] = vector4d(mdl_light[4]_forward.xyz,
						   //								cos((mdl_light[4]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_6lights_depth[] = // 6 model lights
						   //  program.local[0] = lightdir
						   //  program.local[1] = ambientlight
						   //  program.local[2] = diffuselight
						   //  program.local[3-6] = modelview matrix
						   ...
						   //  program.local[26] = vector4d(mdl_light[4]_forward.xyz,
						   //								cos((mdl_light[4]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
						   //  program.local[24] = vector4d(mdl_light[5]_origin.xyz, 1)
						   //  program.local[25] = vector4d(mdl_light[5]_color.xyz, mdl_light[5]_radius)
						   //  program.local[26] = vector4d(mdl_light[5]_forward.xyz,
						   //								cos((mdl_light[5]->spotcos * 2) * 0.3 * (M_PI * 2 / 360))
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_basic_radial[] = // no model light with radial fog
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_1light_radial[] = // 1 model light with radial fog
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_2lights_radial[] = // 2 model lights with radial fog
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_3lights_radial[] = // 3 model lights with radial fog
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_4lights_radial[] = // 4 model lights with radial fog
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_5lights_radial[] = // 5 model lights with radial fog
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_6lights_radial[] = // 6 model lights with radial fog
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"TEMP R2;"
...
char arb_frag_reg[] = // model light fragment shader
	"!!ARBfp1.0"
...
char arb_frag_fog[] = // model light fragment shader with fog
	"!!ARBfp1.0"
	"OPTION ARB_precision_hint_fastest;"
...
// todo: do bone vertex calculations on the gpu,
// this shouldn't be too hard for someone not as lazy as me

const char* modelbones_vertice =
	R"(
	#version 120
	uniform mat4 boneMatrices[128];

	uniform vec3 inPosition;
	uniform int inBoneIndex;

	void main() {
		vec4 skinned = boneMatrices[inBoneIndex] * vec4(inPosition, 1.0);
		gl_Position = (glProjectionMatrix * glModelViewMatrix) * glModelViewMatrix * skinned;
	}
	
	)";

const char* modelbones_fragment =
	R"(
	#version 120
...
void CStudioModelRenderer::Init(void)
{
	...
	m_pCvarViewmodelFov = gEngfuncs.pfnRegisterVariable("viewmodel_fov", "70", FCVAR_ARCHIVE); // best matches the half-life 2 viewmodel fov // I thought H-L:2 had it set to 54
...
void CStudioModelRenderer::StudioResetTextures()
{
	memset(m_pStudioModels, 0, sizeof(m_pStudioModels));
	m_iNumStudioModels = 0;
}
...
void CStudioModelRenderer::SetViewmodelFovProjection(void)
{
	...
	*/
	...
	float fovRad = m_pCvarViewmodelFov->value * static_cast<float>(M_PI) / 180.0f;
	...
	gHUD.viewFrustum.SetFrustum(gBSPRenderer.m_vViewAngles, gBSPRenderer.m_vRenderOrigin, gHUD.m_iFOV, gHUD.m_pFogSettings.end, true);
}
...
void CStudioModelRenderer::StudioDrawWireframe(void)
{
	...
	while (pnext)
	{
		for (int i = 0; i < pnext->numverts; i++)
			VectorTransform(pnext->verts[i].position, (*m_pbonetransform)[pnext->verts[i].boneindex], m_vVertexTransform[i]);
		...
		for (int j = 0; j < pbodypart[i].nummodels; j++)
		{
			...
			for (int m = 0; m < m_pSubModel[j].nummesh; m++)
			{
				for (int n = 0; n < pmeshes[m].numnorms; n++, pnormbone++)
				{
					...
					if ((ptexture[pskinref[meshskinref]].flags & STUDIO_NF_CHROME) && !(pbones[*pnormbone].flags & STUDIO_HAS_CHROME))
					{
		...
		for (int j = 0; j < pbodypart[i].nummodels; j++)
		{
			...
			for (int m = 0; m < m_pSubModel[j].nummesh; m++)
			{
				for (int n = 0; n < pmeshes[m].numnorms; n++, pnormbone++)
				{
					...
					if ((ptexture[pskinref[meshskinref]].flags & STUDIO_NF_CHROME) && !(pbones[pnormbone[0]].flags & STUDIO_HAS_CHROME))
						pbones[*pnormbone].flags |= STUDIO_HAS_CHROME;
				}
			}
		}
...
void CStudioModelRenderer::StudioChromeForMesh(int j, mstudiomesh_t* pmesh)
{
	...
	{
		// taken from edgarbarney/halflife-franbase/
		n = DotProduct(pstudionorms[k], m_vChromeRight[pnormbone[k]]);
		...
		n = DotProduct(pstudionorms[k], m_vChromeUp[pnormbone[k]]);
...
void CStudioModelRenderer::StudioDrawPoints(void)
{
	...
	for (int i = 0; i < m_pSubModel->numnorms; i++)
		VectorRotate(pstudionorms[i], (*m_pbonetransform)[pnormbone[i]], m_pNormalTransform[i]);
	...
	if (!m_bChromeShell)
	{
		...
		for (int i = 0; i < m_pSubModel->numverts; i++)
			VectorTransform(pstudioverts[i], (*m_pbonetransform)[pvertbone[i]], m_pVertexTransform[i]);
...
qboolean CStudioModelRenderer::StudioCheckBBox(void)
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		VectorRotate(vTemp, (*m_protationmatrix), vBounds[i]);
