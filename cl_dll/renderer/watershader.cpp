...
const char* water_depth_vertex =
	...
	void main() {
		R0 = gl_Vertex * modelMatrix)
		R1 = R0 * projectionMatrix;

		gl_Position = R1;

		texCoord0 = vertexTexCoord / 128;

		// Pass finalPosition as texcoord[1]
		texCoord1 = R1;

		// Pass original position (xyz only) as texcoord[2]
		texCoord2 = vertexPosition.xyz;
...
const char* water_fragment_abovewater_regular =
	R"(
	#version 120
	...
	uniform vec3 renderorigin, waterfog;
	uniform float m_flFresnelTerm, flTime;

	uniform sampler2D watertexture, waterrefract, waterreflect;
	...
	void main() {
		R1 = vec4(0.2, 0.15000001, 0.13, 0.11);
		R0.zw = R1.xyxy * fltime + texCoord0.xyxy;
		R0.y = R1.w * fltime + texCoord0;
		R0.x = R1.z * fltime + texCoord0;
		R1.xyz = texture2D(watertexture, R0).xyz;
		R0.xyz = texture2D(watertexture, R0.zwzw).xyz;
		R2.xyz = R0.xyz + R1.xyz;
		R0.xyz = vec3(0.17, 0.14, 0.16);
		R1.xy = R0.yzzw * fltime + texCoord0;
		R0.z = R0.x * fltime + texCoord0.x;
		R0.xyz = texture2D(watertexture, R0.zwzw).xyz;
		R0.xyz += R2.xyz;
		R1.xyz = texture2D(watertexture, R1);
		R0.xyz += R1.xyz;
		R1.xyz = -texCoord2 + renderorigin;
		R1.w = R1 * R1;
		R1.w = 1.0 / sqrt(R1.w);
		R0.xyz *= 0.5;
		R0.xyz += 1;
		...
		R0.xy = R0.z * R0;
		R2.xy = R0 * 0.23;
		R2.z = 1.0 / texCoord1.w;
		R0.xy = R2.z * texCoord1;
		R0.xy = R0 * 0.5 + R2;
		R0.xy += 0.5;
		R1.z *= R1.w;
		R0 = texture2D(waterrefract, R0);
		R1.y = -texCoord1;
		R1.x = texCoord1;
		R1.xy *= R2.z;"
		R1.xy = R1.xy * 0.5 + R2;
		R2.x = R1.z * m_flfresnelterm;
		R1.xy += 0.5;
		R1 = texture2D(waterreflect, R1);
		R2.x *= 1.3;
		R0 += -R1;
		R2.x = min(R2, 0.9700000	3); //the fuck is MIN_SAT? probably just a optimized version of min..?
		R1 += R2.x * R0;
		R0.x = R1.x + R1.y;
		R0.x += R1.z;
		R0.xyz = R0.x * waterfog;
		R0.w = 1;
		R0.xyz *= 0.33333334;
		R0 += -R1;
		gl_FragColor = R0 * 0.2 + R1;
	}
...
//	ARB SHADERS
//===========================================
char water_vp_depth[] = //water vertex program, depth
//program.local[0 - 3] = modelview matrix
//program.local[4 - 7] = projection matrix
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
...
char water_vp_radial[] = //water vertex program, radial fog
	"!!ARBvp1.0"
	"TEMP R0;"
	"TEMP R1;"
	"DP4 R0.x, vertex.position, program.local[0];"
...
char water_fp_aw_reg[] = //water fragment program, above water, regular
//program.local[0] = renderorigin (x, y, z)
//program.local[1] = waterfog (r, g, b)
//program.local[2] = m_flFresnelTerm (float)
//program.local[3] = fltime (float) (client time)
	"!!ARBfp1.0"
...
char water_fp_aw_fog[] = //water fragment program, above water, fog
	"!!ARBfp1.0"
	"OPTION ARB_precision_hint_fastest;"
...
char water_fp_uw_reg[] = //water fragment program, underwater, regular
// program.local[0] = waterfog (r, g, b)
// program.local[1] = fltime (float) (client time)
	"!!ARBfp1.0"
	"OPTION ARB_precision_hint_fastest;"
	...
	//R2.xw = vec4d(R1.y, R1.y, R1.z, R1.z) * 0.5 + vec4d(texcoord.y, texcoord.y, texcoord.z, texcoord.x)
...
char water_fp_uw_fog[] = //water fragment program, underwater, fog
	"!!ARBfp1.0"
	"OPTION ARB_precision_hint_fastest;"
	"OPTION ARB_fog_linear;"
	"PARAM c[6] = { program.local[0..1],"
