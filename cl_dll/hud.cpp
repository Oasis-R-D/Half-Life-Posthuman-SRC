...
int __MsgFunc_FlashHUD(const char* pszName, int iSize, void* pbuf)
{
	return static_cast<int>(gHUD.MsgFunc_FlashHUD(pszName, iSize, pbuf));
}
...
int __MsgFunc_TempEnt(const char* pszName, int iSize, void* pbuf)
{
...
void CHud::Init()
{
	...
	CVAR_CREATE("cl_innacuracydebug", "0", FCVAR_SPONLY);
