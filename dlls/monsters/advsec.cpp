...
class CAdvSec : public CSquadMonster
{
public:
	void Spawn() override;
...
LINK_ENTITY_TO_CLASS(monster_advsec, CAdvSec);

TYPEDESCRIPTION CAdvSec::m_SaveData[] =
	{
		DEFINE_FIELD(CAdvSec, m_flNextGrenadeCheck, FIELD_TIME),
		DEFINE_FIELD(CAdvSec, m_flNextPainTime, FIELD_TIME),
		//	DEFINE_FIELD( CAdvSec, m_flLastEnemySightTime, FIELD_TIME ), // don't save, go to zero
		DEFINE_FIELD(CAdvSec, m_vecTossVelocity, FIELD_VECTOR),
		DEFINE_FIELD(CAdvSec, m_fThrowGrenade, FIELD_BOOLEAN),
		DEFINE_FIELD(CAdvSec, m_fStanding, FIELD_BOOLEAN),
		DEFINE_FIELD(CAdvSec, m_fFirstEncounter, FIELD_BOOLEAN),
		DEFINE_FIELD(CAdvSec, m_cClipSize, FIELD_INTEGER),
		DEFINE_FIELD(CAdvSec, m_voicePitch, FIELD_INTEGER),
		//  DEFINE_FIELD( CShotgun, m_iBrassShell, FIELD_INTEGER ),
		//  DEFINE_FIELD( CShotgun, m_iShotgunShell, FIELD_INTEGER ),
		DEFINE_FIELD(CAdvSec, m_iSentence, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CAdvSec, CSquadMonster);

const char* CAdvSec::pGruntSentences[] =
	{
...
// CheckMeleeAttack1
//=========================================================
bool CAdvSec::CheckMeleeAttack1(float flDot, float flDist)
{
...
//=========================================================
bool CAdvSec::CheckRangeAttack1(float flDot, float flDist)
{
...
// attack.
//=========================================================
bool CAdvSec::CheckRangeAttack2(float flDot, float flDist)
{
...
//=========================================================
void CAdvSec::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
...
void CAdvSec::IdleSound()
{
	if (FOkToSpeak() && (0 != g_fAdvSecQuestion || RANDOM_LONG(0, 1)))
	{
		if (0 == g_fAdvSecQuestion)
		{
...
void CAdvSec::Shoot()
{
	...
	if (g_iSkillLevel != SKILL_REALISM)
		CPhysbullet::BulletCreate(1, gSkillData.monDmgMP5, 6297.6f, vecShootOrigin, vecShootDir, CONE_3DEGREES, CONE_6DEGREES, 0.66, 9, edict());
	else
...
//=========================================================
void CAdvSec::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	Vector vecShootDir;
...
// start task
//=========================================================
void CAdvSec::StartTask(Task_t* pTask)
{
...
// RunTask
//=========================================================
void CAdvSec::RunTask(Task_t* pTask)
{
	switch (pTask->iTask)
	{
	case TASK_GRUNT_FACE_TOSS_DIR:
...
Schedule_t slAdvSecFail[] =
	{
		{tlAdvSecFail,
			ARRAYSIZE(tlAdvSecFail),
...
Schedule_t slAdvSecCombatFail[] =
	{
		{tlAdvSecCombatFail,
			ARRAYSIZE(tlAdvSecCombatFail),
			bits_COND_CAN_RANGE_ATTACK1 |
				bits_COND_CAN_RANGE_ATTACK2,
			0,
			"Grunt Combat Fail"},
...
Schedule_t slAdvSecVictoryDance[] =
	{
		{tlAdvSecVictoryDance,
			ARRAYSIZE(tlAdvSecVictoryDance),
			bits_COND_NEW_ENEMY |
...
// the grunt to attack.
//=========================================================
Task_t tlAdvSecEstablishLineOfFire[] =
	{
...
Schedule_t slAdvSecEstablishLineOfFire[] =
	{
		{tlAdvSecEstablishLineOfFire,
			ARRAYSIZE(tlAdvSecEstablishLineOfFire),
			bits_COND_NEW_ENEMY |
...
Schedule_t slAdvSecFoundEnemy[] =
	{
		{tlAdvSecFoundEnemy,
			ARRAYSIZE(tlAdvSecFoundEnemy),
			bits_COND_HEAR_SOUND,
...
Schedule_t slAdvSecCombatFace[] =
	{
		{tlAdvSecCombatFace1,
			ARRAYSIZE(tlAdvSecCombatFace1),
			bits_COND_NEW_ENEMY |
				bits_COND_ENEMY_DEAD |
				bits_COND_CAN_RANGE_ATTACK1 |
				bits_COND_CAN_RANGE_ATTACK2,
...
Schedule_t slAdvSecSignalSuppress[] =
	{
		{tlAdvSecSignalSuppress,
			ARRAYSIZE(tlAdvSecSignalSuppress),
			bits_COND_ENEMY_DEAD |
				bits_COND_LIGHT_DAMAGE |
				bits_COND_HEAVY_DAMAGE |
				bits_COND_HEAR_SOUND |
...
Schedule_t slAdvSecSuppress[] =
	{
		{tlAdvSecSuppress,
			ARRAYSIZE(tlAdvSecSuppress),
			bits_COND_ENEMY_DEAD |
				bits_COND_LIGHT_DAMAGE |
				bits_COND_HEAVY_DAMAGE |
				bits_COND_HEAR_SOUND |
...
Schedule_t slAdvSecWaitInCover[] =
	{
		{tlAdvSecWaitInCover,
			ARRAYSIZE(tlAdvSecWaitInCover),
			bits_COND_NEW_ENEMY |
				bits_COND_HEAR_SOUND |
				bits_COND_CAN_RANGE_ATTACK1 |
				bits_COND_CAN_RANGE_ATTACK2 |
...
Schedule_t slAdvSecTakeCover[] =
	{
		{tlAdvSecTakeCover1,
			ARRAYSIZE(tlAdvSecTakeCover1),
			0,
			0,
			"TakeCover"},
};
...
Schedule_t slAdvSecGrenadeCover[] =
	{
		{tlAdvSecGrenadeCover1,
			ARRAYSIZE(tlAdvSecGrenadeCover1),
			0,
			0,
			"GrenadeCover"},
};
...
// drop grenade then run to cover.
//=========================================================
Task_t tlAdvSecTossGrenadeCover1[] =
	{
		{TASK_FACE_ENEMY, (float)0},
...
Schedule_t slAdvSecTossGrenadeCover[] =
	{
		{tlAdvSecTossGrenadeCover1,
			ARRAYSIZE(tlAdvSecTossGrenadeCover1),
			0,
			0,
			"TossGrenadeCover"},
};
...
//=========================================================
Task_t tlAdvSecTakeCoverFromBestSound[] =
	{
...
Schedule_t slAdvSecTakeCoverFromBestSound[] =
	{
		{tlAdvSecTakeCoverFromBestSound,
			ARRAYSIZE(tlAdvSecTakeCoverFromBestSound),
			0,
			0,
...
Schedule_t slAdvSecHideReload[] =
	{
		{tlAdvSecHideReload,
			ARRAYSIZE(tlAdvSecHideReload),
			bits_COND_HEAVY_DAMAGE |
				bits_COND_HEAR_SOUND,
...
Schedule_t slAdvSecSweep[] =
	{
		{tlAdvSecSweep,
			ARRAYSIZE(tlAdvSecSweep),
...
Schedule_t slAdvSecRangeAttack1A[] =
	{
		{tlAdvSecRangeAttack1A,
			ARRAYSIZE(tlAdvSecRangeAttack1A),
			bits_COND_NEW_ENEMY |
				bits_COND_ENEMY_DEAD |
				bits_COND_HEAVY_DAMAGE |
...
Schedule_t slAdvSecRangeAttack1B[] =
	{
		{tlAdvSecRangeAttack1B,
			ARRAYSIZE(tlAdvSecRangeAttack1B),
			bits_COND_NEW_ENEMY |
				bits_COND_ENEMY_DEAD |
				bits_COND_HEAVY_DAMAGE |
				bits_COND_ENEMY_OCCLUDED |
...
Schedule_t slAdvSecRangeAttack2[] =
	{
		{tlAdvSecRangeAttack2,
			ARRAYSIZE(tlAdvSecRangeAttack2),
			0,
			0,
			"RangeAttack2"},
};
...
Schedule_t slAdvSecRepel[] =
	{
		{tlAdvSecRepel,
			ARRAYSIZE(tlAdvSecRepel),
			bits_COND_SEE_ENEMY |
				bits_COND_NEW_ENEMY |
				bits_COND_LIGHT_DAMAGE |
				bits_COND_HEAVY_DAMAGE |
...
Schedule_t slAdvSecRepelAttack[] =
	{
		{tlAdvSecRepelAttack,
			ARRAYSIZE(tlAdvSecRepelAttack),
			bits_COND_ENEMY_OCCLUDED,
			0,
			"Repel Attack"},
};
...
Schedule_t slAdvSecRepelLand[] =
	{
		{tlAdvSecRepelLand,
			ARRAYSIZE(tlAdvSecRepelLand),
			bits_COND_SEE_ENEMY |
				bits_COND_NEW_ENEMY |
				bits_COND_LIGHT_DAMAGE |
				bits_COND_HEAVY_DAMAGE |
...
DEFINE_CUSTOM_SCHEDULES(CAdvSec){
	slAdvSecFail,
	slAdvSecCombatFail,
	slAdvSecVictoryDance,
	slAdvSecEstablishLineOfFire,
	slAdvSecFoundEnemy,
	slAdvSecCombatFace,
	slAdvSecSignalSuppress,
	slAdvSecSuppress,
	slAdvSecWaitInCover,
	slAdvSecTakeCover,
	slAdvSecGrenadeCover,
	slAdvSecTossGrenadeCover,
	slAdvSecTakeCoverFromBestSound,
	slAdvSecHideReload,
	slAdvSecSweep,
	slAdvSecRangeAttack1A,
	slAdvSecRangeAttack1B,
	slAdvSecRangeAttack2,
	slAdvSecRepel,
	slAdvSecRepelAttack,
	slAdvSecRepelLand,
};

IMPLEMENT_CUSTOM_SCHEDULES(CAdvSec, CSquadMonster);

//=========================================================
// SetActivity
//=========================================================
void CAdvSec::SetActivity(Activity NewActivity)
{
...
// Get Schedule!
//=========================================================
Schedule_t* CAdvSec::GetSchedule()
{
...
//=========================================================
//=========================================================
Schedule_t* CAdvSec::GetScheduleOfType(int Type)
{
	switch (Type)
	{
	case SCHED_TAKE_COVER_FROM_ENEMY:
	{
		if (InSquad())
		{
			if (g_iSkillLevel >= SKILL_HARD && HasConditions(bits_COND_CAN_RANGE_ATTACK2) && OccupySlot(bits_SLOTS_HGRUNT_GRENADE))
			{
				...
				return slAdvSecTossGrenadeCover;
			}
			else
			{
				return &slAdvSecTakeCover[0];
			}
		}
		else
		{
			...
			{
				return &slAdvSecGrenadeCover[0];
			}
		}
	}
	case SCHED_TAKE_COVER_FROM_BEST_SOUND:
	{
		return &slAdvSecTakeCoverFromBestSound[0];
	}
	...
	{
		return &slAdvSecEstablishLineOfFire[0];
	}
	break;
	case SCHED_RANGE_ATTACK1:
	{
		...
		if (m_fStanding)
			return &slAdvSecRangeAttack1B[0];
		else
			return &slAdvSecRangeAttack1A[0];
	}
	case SCHED_RANGE_ATTACK2:
	{
		return &slAdvSecRangeAttack2[0];
	}
	case SCHED_COMBAT_FACE:
	{
		return &slAdvSecCombatFace[0];
	}
	case SCHED_GRUNT_WAIT_FACE_ENEMY:
	{
		return &slAdvSecWaitInCover[0];
	}
	case SCHED_GRUNT_SWEEP:
	{
		return &slAdvSecSweep[0];
	}
	case SCHED_GRUNT_COVER_AND_RELOAD:
	{
		return &slAdvSecHideReload[0];
	}
	case SCHED_GRUNT_FOUND_ENEMY:
	{
		return &slAdvSecFoundEnemy[0];
	}
	case SCHED_VICTORY_DANCE:
	{
		if (InSquad())
		{
			if (!IsLeader())
			{
				return &slAdvSecFail[0];
			}
		}

		return &slAdvSecVictoryDance[0];
	}
	case SCHED_GRUNT_SUPPRESS:
	{
		if (m_hEnemy->IsPlayer() && m_fFirstEncounter)
		{
			...
			return &slAdvSecSignalSuppress[0];
		}
		else
		{
			return &slAdvSecSuppress[0];
		}
	}
	case SCHED_FAIL:
	{
	...
	case SCHED_GRUNT_REPEL:
	{
		if (pev->velocity.z > -128)
			pev->velocity.z -= 32;
		return &slAdvSecRepel[0];
	}
	case SCHED_GRUNT_REPEL_ATTACK:
	{
		if (pev->velocity.z > -128)
			pev->velocity.z -= 32;
		return &slAdvSecRepelAttack[0];
	}
	case SCHED_GRUNT_REPEL_LAND:
	{
		return &slAdvSecRepelLand[0];
	}
	default:
	{
...
class CAdvSecRepel : public CBaseMonster
{
public:
	void Spawn() override;
	void Precache() override;
...
LINK_ENTITY_TO_CLASS(monster_advsec_repel, CAdvSecRepel);

void CAdvSecRepel::Spawn()
{
	Precache();
	pev->solid = SOLID_NOT;

	SetUse(&CAdvSecRepel::RepelUse);
}

void CAdvSecRepel::Precache()
{
	UTIL_PrecacheOther("monster_human_grunt");
	m_iSpriteTexture = PRECACHE_MODEL("sprites/rope.spr");
}

void CAdvSecRepel::RepelUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	TraceResult tr;
...
const char* CDeadAdvSec::m_szPoses[] = {"deadstomach", "deadside", "deadsitting"};

bool CDeadAdvSec::KeyValue(KeyValueData* pkvd)
{
