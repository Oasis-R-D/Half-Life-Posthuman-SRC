...
*			Panthereye AKA Diablo
 ...
 Modified and adjusted by PackMan for Half-Life: Post-Human
*															*
************************************************************/
...
{
	return FBitSet(pev->flags, FL_ONGROUND) && flDist > 80.0f && flDist <= 256 && flDot >= 0.65f && NextLeapTime <= gpGlobals->time;
}
...
void CDiablo::StrikeSFX(bool hit)
{
	if (hit)
	{
...
Schedule_t* CDiablo::GetSchedule()
{
	...
	case MONSTERSTATE_COMBAT:
	{
		...
		if (InSquad() && HasConditions(bits_COND_LIGHT_DAMAGE))
		{
			// if hurt:
			// 30% chance of taking cover
			// 20% chance of flinch.
			int iPercent = RANDOM_LONG(0, 99);

			if (iPercent <= 30 && m_hEnemy != NULL)
...
void CDiablo::StartTask(Task_t* pTask)
{
	...
	case TASK_DIABLO_DETERMINE_UNSTUCK_IDEAL: {
		...
		for (i = 0; i < 8; i++) 
		{
			...
			// TO-DO: this classname getting seems useless
			const char* hitClassname = "NULL";
			float fracto = tr.flFraction;
			if (tr.pHit != NULL && tr.pHit->v.classname != NULL) 
			{
