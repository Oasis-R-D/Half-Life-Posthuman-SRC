...
bool CBasePlayer::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	while (fTookDamage && (!ftrivial || (bitsDamage & DMG_TIMEBASED) != 0) && ffound && 0 != bitsDamage)
	{
		...
		{
			SetSuitUpdate("!HEV_FIRE", false, SUIT_NEXT_IN_1MIN); // fire detected
			bitsDamage &= ~DMG_BURN;
			ffound = true;
		}
	}
...
void CBasePlayer::WaterMove()
{
	...
	if (pev->waterlevel != 3)
	{
		...
		if (pev->air_finished < gpGlobals->time)
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/wade1.wav", 1, ATTN_NORM);
		else if (pev->air_finished < gpGlobals->time + 9)
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/wade2.wav", 1, ATTN_NORM);
...
void CBasePlayer::PlayerUse()
{
	...
	{
		//Special behavior for ropes: check if the player is close enough to the rope segment origin // ROPEUPD
	...
	else
	{
		if ((m_afButtonPressed & IN_USE) != 0)
		{
			if (HasSuit())
			{
				...
				EMIT_SOUND(ENT(pev), CHAN_ITEM, "common/wpn_denyselect.wav", RANDOM_FLOAT(0.4, 0.5), ATTN_NORM);
		}
	}
}
...
void CBasePlayer::PreThink()
{
	...
	//We're on a rope. - Solokiller // ROPEUPD
...
void CBasePlayer::PostThink()
{
	...
	if ((FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD)
	{
		...
		if (pev->watertype == CONTENT_WATER)
		{
			...
			// EMIT_SOUND(ENT(pev), CHAN_BODY, "player/wade1.wav", 1, ATTN_NORM);
		}
...
bool IsSpawnPointValid(CBaseEntity* pPlayer, CBaseEntity* pSpot)
{
	...
	while ((ent = UTIL_FindEntityInSphere(ent, pSpot->pev->origin, 512)) != NULL)
	{
...
void CBasePlayer::CheatImpulseCommands(int iImpulse)
{
	...
	case 101:
		...
		GiveNamedItem("weapon_headcrab");
		GiveNamedItem("weapon_headcrab_fast");
		GiveNamedItem("weapon_headcrab_poison");
		GiveNamedItem("weapon_spitthrower");
...
{
	CVAR_SET_FLOAT("sv_maxvelocity", 9999); // ugly but required for crossbow bolts
...
void CDeadHEV::Spawn()
{
	PRECACHE_MODEL("models/survey.mdl");
	SET_MODEL(ENT(pev), "models/survey.mdl");
