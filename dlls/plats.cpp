...
TYPEDESCRIPTION CFuncTrain::m_SaveData[] =
	{
		...
		DEFINE_FIELD(CFuncTrain, m_explodedmg, FIELD_INTEGER),
};
...
bool CFuncTrain::KeyValue(KeyValueData* pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "explodedmg"))
	{
		m_explodedmg = atoi(pkvd->szValue);
		return true;
	}
	...
	if (m_explodedmg > 0)
	{
		...
		while ((pEntity = UTIL_FindEntityInSphere(pEntity, pev->origin, 64)) != NULL)
		{
			if (pEntity->IsPlayer())
			{
				...
				WRITE_BYTE((m_explodedmg - 50) * .60); // scale * 10
				WRITE_BYTE(15);						   // framerate
				WRITE_BYTE(TE_EXPLFLAG_NONE);
				MESSAGE_END();

				CSoundEnt::InsertSound(bits_SOUND_COMBAT, pev->origin, NORMAL_EXPLOSION_VOLUME, 3.0);
				RadiusDamage(pev->origin, pev, pev, m_explodedmg, m_explodedmg * 2.5, CLASS_NONE, DMG_BLAST);

				if (iContents != CONTENTS_WATER)
				{
					int sparkCount = RANDOM_LONG(0, 3);
					for (int i = 0; i < sparkCount; i++)
						Create("spark_shower", pev->origin, UTIL_GetGlobalTrace().vecPlaneNormal, NULL);
				}
				...
				// # of shards
				WRITE_BYTE(m_explodedmg / 10); // let client decide
				// duration
...
void CFuncTrackTrain::Precache()
{
	...
	case 6:
		...
		break;
	case 7:
		PRECACHE_SOUND("plats/c1a0b_move1.wav");
		pev->noise = MAKE_STRING("plats/c1a0b_move1.wav");
		break;
	case 8:
		PRECACHE_SOUND("plats/c1a0b_move2.wav");
		pev->noise = MAKE_STRING("plats/c1a0b_move2.wav");
		break;
	}
