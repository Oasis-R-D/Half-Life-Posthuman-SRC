...
void CBaseEntity::BulletRic(entvars_t* pevAttacker, Vector vecDir, TraceResult* ptr, int bitsDamageType, CBaseEntity* Attacked) // easier way to handle ricochet
{
	if ((bitsDamageType & (DMG_BULLET | DMG_BLAST)) != 0)
	{
		...
		if (BULLET != nullptr)
		{
			...
			CPhysbullet::BulletCreate(1, round(BULLET->m_BulletDamage/2), BULLET->m_muzzlevelocity, spawnpos, Dir, CONE_60DEGREES, CONE_60DEGREES, BULLET->m_Gravity, BULLET->m_Flare, edict(), false, 0);
			UTIL_Remove(BULLET);
		}
	}
}
	...
	for (unsigned int iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			else
			{
				switch (iBulletType)
				{
				case BULLET_PLAYER_BUCKSHOT: // make distance based!
				...
				case BULLET_PLAYER_M727:
					pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgM727, vecDir, &tr, DMG_BULLET);
					break;
				}

				TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
				DecalGunshot(&tr, iBulletType);
				UTIL_BloodPuff(tr, pEntity->BloodColor());
			}
		}
		// make bullet trails
...
void CBaseEntity::TraceBleed(float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
	...
	for (i = 0; i < cCount; i++)
	{
		...
		UTIL_TraceLine(ptr->vecEndPos, ptr->vecEndPos + vecTraceDir * -128, ignore_monsters, ENT(pev), &Bloodtr);
