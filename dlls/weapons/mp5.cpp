...
LINK_ENTITY_TO_CLASS(weapon_9mmAR_dual, CMP5Dual);
bool CMP5Dual::CanAttack(float attack_time, float curtime, bool isPredicted)
{
...
bool CMP5Dual::GetItemInfo(ItemInfo* p)
{
	p->pszName = STRING(pev->classname);
	p->pszAmmo1 = "9mm";
	p->iMaxAmmo1 = _9MM_MAX_CARRY;
...
void CMP5Dual::Spawn()
{
	pev->classname = MAKE_STRING("weapon_9mmAR_dual"); // hack to allow for old names
	Precache();
	m_iId = WEAPON_MP5DUAL;
	SET_MODEL(ENT(pev), "models/w_9mmAR.mdl");
...
{
	PRECACHE_MODEL("models/v_9mmarR.mdl");
	PRECACHE_MODEL("models/v_9mmarL.mdl");
	PRECACHE_MODEL("models/w_9mmAR.mdl");
	PRECACHE_MODEL("models/p_9mmAR.mdl");
	PRECACHE_MODEL("models/grenade.mdl"); // grenade
	PRECACHE_MODEL("models/w_9mmARclip.mdl");
...
bool CMP5Dual::Deploy()
{
	...
	return DefaultDeploy("models/v_9mmarR.mdl", "models/p_9mmAR.mdl", MP5_DRAW, "mp5", 0, "models/v_9mmarL.mdl", MP5_DRAW);
}


void CMP5Dual::Holster()
{
	m_flAccuracyPenalty = 0.0;
	...
	SendWeaponAnim(MP5_HOLSTER, 0 , true);
}

void CMP5Dual::SecondaryAttack()
{
	// don't fire underwater
	if (m_pPlayer->pev->waterlevel == 3)
	{
	...
	if (g_iSkillLevel != SKILL_HARD)
	{
		...
		m_flNextSecondaryAttack = 0.066;
	}
	else
	{
		m_flNextSecondaryAttack = 0.075;
	}
...
void CMP5Dual::PrimaryAttack()
{
	...
	SendWeaponAnim(RANDOM_LONG(MP5_SHOOT1, MP5_SHOOT3), 0, true);
...
void CMP5Dual::TertiaryAttack()
{

}

void CMP5Dual::ItemPreFrame()
{
	// Check our penalty time decay
	if ( ( m_flTimeSincePrimary + m_flNextPrimaryAttack < gpGlobals->time ) )
	{
...
const Vector& CMP5Dual::GetBulletSpread()
{		
	static Vector cone;
...
void CMP5Dual::Shoot(int gunnumb)
{
	...
	if (g_iSkillLevel != SKILL_HARD)
	{
		CPhysbullet::BulletCreate(1, gSkillData.plrDmgMP5, 6000, vecSrc, vecAiming, spread, spread, 0.66, 9, m_pPlayer->edict());
	}
	else
	{
...
void CMP5Dual::Reload()
{
	DefaultReload(60, m_iCylR_ammo == 0 ? MP5_RELOAD_EMPTY : MP5_RELOAD_TACTICAL, 1.5, 0, true, m_iCylL_ammo == 0 ? MP5_RELOAD_EMPTY : MP5_RELOAD_TACTICAL);
}


void CMP5Dual::WeaponIdle()
{
	ResetEmptySound();
	...
	{
		if (RANDOM_LONG(0, 1))
			SendWeaponAnim(MP5_IDLE1), m_flTimeAltWeaponIdle = 5.37;
		else
			SendWeaponAnim(MP5_IDLE2), m_flTimeAltWeaponIdle = 4.5;
	}
}

void CMP5Dual::ItemPostFrame() // completely overriden to make multiple changes
{
