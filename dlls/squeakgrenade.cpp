...
void CSqueak::PrimaryAttack()
{
	if (0 != m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
	{
		...
		if (tr.fAllSolid == 0 && tr.fStartSolid == 0 && tr.flFraction > 0.25)
		{
			...
			Vector angle = Vector(pev->angles.x, 0, pev->angles.z);
...
LINK_ENTITY_TO_CLASS(weapon_headcrab, CHeadcrabGrenade);


void CHeadcrabGrenade::Spawn()
{
	Precache();
	m_iId = WEAPON_GRENADE_HEADCRAB;
...
void CHeadcrabGrenadeFast::Spawn()
{
	Precache();
	m_iId = WEAPON_GRENADE_HEADCRAB_FAST;
...
bool CHeadcrabGrenadeFast::GetItemInfo(ItemInfo* p)
{
	p->pszName = STRING(pev->classname);
	p->pszAmmo1 = "headcrab_fast";
	p->iMaxAmmo1 = 10;
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = WEAPON_NOCLIP;
...
void CHeadcrabGrenadeFast::PrimaryAttack()
{
	if (0 != m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
	{
		UTIL_MakeVectors(m_pPlayer->pev->v_angle);
...
LINK_ENTITY_TO_CLASS(weapon_headcrab_poison, CHeadcrabGrenadePoison);


void CHeadcrabGrenadePoison::Spawn()
{
	Precache();
	m_iId = WEAPON_GRENADE_HEADCRAB_POISON;
...
bool CHeadcrabGrenadePoison::GetItemInfo(ItemInfo* p)
{
	p->pszName = STRING(pev->classname);
	p->pszAmmo1 = "headcrab_poison";
	p->iMaxAmmo1 = 5;
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = WEAPON_NOCLIP;
...
void CHeadcrabGrenadePoison::PrimaryAttack()
{
	if (0 != m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
	{
		UTIL_MakeVectors(m_pPlayer->pev->v_angle);
