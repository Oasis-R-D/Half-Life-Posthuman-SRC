...
void W_Precache()
{
	...
	UTIL_PrecacheOtherWeapon("weapon_headcrab");
	UTIL_PrecacheOtherWeapon("weapon_headcrab_fast");
	UTIL_PrecacheOtherWeapon("weapon_headcrab_poison");
...
void CBasePlayerWeapon::Recoil(float RecoilUp, float RecoilHorz, bool patterned) 
{
	if (CVAR_GET_FLOAT("cl_innacuracydebug") < 2)
	{
		...
		if (m_pPlayer->health_armR == 0 || (m_pPlayer->health_armR < 100 && RANDOM_LONG(0, m_pPlayer->health_armR) / m_pPlayer->health_armR > 0.25))
		{
			m_pPlayer->pev->punchangle.x -= 0.75;
		}

		if (m_pPlayer->health_armL == 0 || (m_pPlayer->health_armL < 100 && RANDOM_LONG(0, m_pPlayer->health_armL) / m_pPlayer->health_armL > 0.1))
		{
...
IMPLEMENT_SAVERESTORE(CM29, CBasePlayerWeapon);
