...
void CPhyscryst::CrystalCreate(int BLLTamnt, float BLLTDamage, int BLLTSpeed, Vector VecSpawnPos, Vector vecDir, float vecSpread, float vecSpreadvert, float BLLTGravity, int CrystType, edict_t *shooter)
{
	for (int i = 0; i < BLLTamnt; i++) // Allows multishot
	{
		...
		pBullet->m_Crysttype = FlareType; // tracer type
		pBullet->Spawn();
