...
int CAGrunt::IRelationship(CBaseEntity* pTarget)
{
	...
	else if (FClassnameIs(pTarget->pev, "monster_advsec")) 
	{
		return R_NM;
	}
...
void CAGrunt::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
	...
	else
	{
		...
		SpawnBlood(ptr->vecEndPos, m_iBloodColor, flDamage); // a little surface blood.
		TraceBleed(flDamage, vecDir, ptr, bitsDamageType);
		#ifndef CLIENT_DLL
				CPhysblood::BloodCreate(BLDAMNT, 350, vecOrigin, vecDir, 1, m_iBloodColor);
		#endif
	}
